...
SELECT MAX(SALARY) AS MAXIMUM_IT FROM EMP WHERE DEPARTMENT='IT';

-- 6. Count employee whose joining date is after 8-feb-91.

SELECT COUNT(EID) AS COUNT FROM EMP WHERE JOININGDATE > '1991-02-08';
...
SELECT SUM(SALARY) AS TOTAL_HR FROM EMP WHERE DEPARTMENT ='HR';
...
SELECT CITY,COUNT(EID) AS COUNT_EMPLOYEE FROM EMP GROUP BY CITY;
...
SELECT COUNT(EID) AS COUNT_RAJKOT FROM EMP WHERE CITY='RAJKOT';
...
SELECT COUNT(EID) AS COUNT_EMPLOYEE FROM EMP WHERE JOININGDATE > '1991-01-01' AND DEPARTMENT='IT';

-- 3. Find the Joining Date wise Total Salaries.

SELECT JOININGDATE,MAX(SALARY) AS MAX_SAL FROM EMP GROUP BY JOININGDATE;
