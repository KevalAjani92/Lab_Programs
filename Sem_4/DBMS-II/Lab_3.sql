...
CREATE OR ALTER PROCEDURE PR_DEPARTMENT_GETEMPBYDEPT
    @DEPTNAME VARCHAR(100)
AS
BEGIN
    SELECT E.FirstName+' '+E.LastName AS EMP_NAME
    FROM Employee E
        JOIN Departments D ON E.DepartmentID = D.DepartmentID
    WHERE D.DepartmentName = @DEPTNAME
END

EXEC PR_DEPARTMENT_GETEMPBYDEPT 'FINANCE';
...
CREATE OR ALTER PROCEDURE PR_PROJECT_GETPROJECTBYPNAME
    @PRONAME VARCHAR(100),
    @DEPTNAME VARCHAR(100)
AS
BEGIN
    ...
    WHERE P.ProjectName = @PRONAME AND D.DepartmentName = @DEPTNAME
END

EXEC PR_PROJECT_GETPROJECTBYPNAME 'PROJECT ALPHA','IT';
...
CREATE OR ALTER PROCEDURE PR_EMPLOYEE_SALARYRANGE
    @MINSAL INT,
    @MAXSAL INT
AS
BEGIN
...
EXEC PR_EMPLOYEE_SALARYRANGE 50000,70000;

-- 5. Create a Procedure that will accepts a date and gives all the employees who all are hired on that date. 

CREATE OR ALTER PROCEDURE PR_EMPLOYEE_GETEMPBYHIREDATE
    @HDATE DATETIME
AS
BEGIN
    SELECT *
...
EXEC PR_EMPLOYEE_GETEMPBYHIREDATE '2012-07-18';
...
CREATE OR ALTER PROCEDURE PR_EMPLOYEE_GETBYNAMEORDEPT
    @FNAME VARCHAR(100) = NULL,
    @DEPTNAME VARCHAR(100) = NULL
AS
BEGIN
    SELECT E.EmployeeID, E.FirstName+' '+E.LastName AS EMP_NAME, E.Gender, E.HireDate, E.Salary, E.DoB, D.DepartmentName
    FROM Employee E
        JOIN Departments D ON E.DepartmentID = D.DepartmentID 
        WHERE E.FirstName = @FNAME OR D.DepartmentName = @DEPTNAME
END         

EXEC PR_EMPLOYEE_GETBYNAMEORDEPT NULL,'HR';
...
EXEC PR_DEPARTMENT_GETBYLOCATION 'W ';
...
CREATE OR ALTER PROCEDURE PR_PROJECT_GETBYDATERANGE
    @SDATE DATETIME,
    @EDATE DATETIME
AS
BEGIN 
...
EXEC PR_PROJECT_GETBYDATERANGE '2021-06-01','2022-05-31';
...
CREATE OR ALTER PROCEDURE PR_DEPARTMENT_GETPROJECT
    @PRONAME VARCHAR(100),
    @LOCATION VARCHAR(100)
AS
BEGIN
    SELECT D.DepartmentName,E.FirstName+' '+E.LastName AS MANAGER_NAME,P.ProjectName,P.StartDate,P.EndDate FROM Departments D
    JOIN Employee E ON D.DepartmentID = E.DepartmentID
    JOIN Projects P ON D.DepartmentID = P.DepartmentID    
    WHERE P.ProjectName = @PRONAME AND D.Location = @LOCATION AND E.EmployeeID = D.ManagerID
END    

EXEC PR_DEPARTMENT_GETPROJECT 'PROJECT ALPHA','NEW YORK';